...
**************************************************************************/
//if button press == true iterate through modes less than reset else reset.
void chooseMode() {
...
**************************************************************************/
// when switching through mode values change led color and break out accordingly...
void setRGB()
{
  switch (mode)
  {
...
void live()
{
 offsetFrequency = getPhotoFrequency();
  ...
  //evaluate the pinVal is greater than noise
...
void recordWithDuration()
{
  ...
  if (countNotes < MAX_NOTES)
  {
    ...
    */
    ...
    {
      // IF NONE OF THE ABOVE IS HAPPENING THEN SHUT OFF BUZZ
      noTone(BUZZER_PIN);
...
{
  //aka time flowing
  // count in miliseconds, elapsed?... assures that time passed start from 0.
  startTime = millis();
  timePassed = 0;
  //serial.print()
  // notify that its been pressed
...
void updateTimer()
{
  // removes miliseconds counted from the begining
...
void playCurrentNote()
{
  ...
  // just reads the value of note being played plus modulation
...
{ 
  ...
  if (countNotes < MAX_NOTES) {
    // frequencies are equal to notes being counted, lenght of notes in array
    // goes through the array saving notes played as well as their durations// count notes only keeps track of notes played
    ...
    countNotes = 0;
    //resets the boolean for array notes and timer reset my time passed to allow for a break
    }
    //my button is not being pressed
...
{
  //give me results of photocell makes it so it returns my frequencies mapping them so they come bac poportional to the color range
...
//counter for running avg, 
 ...
 // calculates the avg sums up the values of array indexes , returns it the divides by the amount of full indexes
...
{
  //allow us to modulate sound
//  offsetFrequency = getPhotoFrequency();
  // count through my array while i is less than count notes
  //as it does not exceed max note play through tones
  if (countNotes < MAX_NOTES) {
    // wait for tone to finish affect color led with every notes from array and go through that array notes and durations
    ...
    //if false start the note count over again
    countNotes = 0;
  }
}
